...
<body>
	...
	<header>
		...
		<div style=" text-align: center; width: 100%; margin-right: auto; margin-left: auto; background: #3C3C3C; padding: 10px 0px 10px 0px; ">
			<p style="font-size: 20px; font-style: italic; color: white;"> Shipment Status</p>
		</div>
		<br>
...
<div class="search">
			<label for="search">SEARCH : </label><input type="text" id="search" onkeyup="myFunction()" placeholder="Shipment Number..." style=" font-size: inherit; text-align: center;padding: 5px;">
	...
	<table id="searcharea">
		...
		<tbody>
			...
			$result = mysqli_query($db, "SELECT * FROM order_detail WHERE ship_status = 'Shipping the Order' AND order_date > DATE_SUB(NOW(), INTERVAL 1 WEEK) ORDER BY order_date DESC");
			...
			if ($result->num_rows > 0) {
				...
				while ($row = mysqli_fetch_assoc($result)) {
					...
					<td data-label='Shipment Order No. :'>" .$row["order_no"]. "</td>" 
					."<td data-label='Account No. :'>" .$row["acc_no"]."</td>" 
					."<td data-label='Shipment Description:'><p>".$row["order_desc"]."</p></td>"
					."<td data-label='Shipment Status'>" .$row["ship_status"]."</td>"
					."<td data-label='Action'>
					<br><br>
					<a href='update_ship.php?ship_id=$order_id' name='done' style='width: 50%; background: black; padding: 10px; color:white; text-decoration: none; border-radius: 5px;'>Done</a>
					<br>	<br>
				...
				echo '"<center><h3>"No Shipments For Today"';
				echo "<br>";
				echo "<br>";
			}
		...
		<div style=" text-align: center; width: 100%; margin-right: auto; margin-left: auto; background: #3C3C3C; padding: 10px 0px 10px 0px; ">
			<p style="font-size: 20px; font-style: italic; color: white;"> Shipment History</p>
		</div>
		<br>
		...
		<tbody>
			...
			$result = mysqli_query($db, "SELECT * FROM order_detail WHERE ship_status = 'ALREADY DELIVERED' AND archive_status = 'NOT ARCHIVED' ORDER BY order_date LIMIT 0,3");
			...
			if ($result->num_rows > 0) {
				...
				while ($row = mysqli_fetch_assoc($result)) {
					...
					."<td data-label='Action'>
					<br><br>
					<a href='delete_ship.php?ship_no=$order_id' name='done' style='width: 50%; background: black; padding: 10px; color:white; text-decoration: none; border-radius: 5px;'>ARCHIVE</a>
					<br>	<br>
				...
				echo '"<center><h3>"All Shipments History Has Been Archived"';
				echo "<br>";
				echo "<br>";
			}
