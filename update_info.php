    ...
    exit();
   	...
   	if (isset($_GET['logout'])) {
    	...
    	$db = new mysqli('localhost', 'root', '', 'healthyburst');
    		$event = mysqli_query($db, "INSERT INTO event_log( event_user, event_activty, event_timestamp) VALUES ('".$_SESSION["email"]."','Sign-Out',current_timestamp())");
...
main{
	...
	main section .user_info table td input[type=text], main section .user_info table td input[type=date]{
		padding: 5px;
		width: 100%;
		text-align: center;
	}
...
.info-header p{
	...
	main section .user_info table td input[type=submit]{
		width: 40%;
		padding: 3px;
		margin-top: 20px;
	}
    ...
    if($_SERVER["REQUEST_METHOD"]=="POST")
{
    if (isset($_POST['update'])) {
    	...
    	mysqli_query($db, "INSERT INTO event_log( event_user, event_activty, event_timestamp) VALUES ('".$_SESSION["email"]."','Updated Profile',current_timestamp())");
    	$result_update = mysqli_query($db, "UPDATE account SET 
    		fname ='$fname',
    		lname ='$lname',
    		email ='$email',
    		phonenumber ='$phonenumber',
    		address ='$address',
    		birthday ='$birthday' WHERE acc_no= '$acc_no'");
	...
	<form method="POST" action="update_info.php">
		...
		<table>
				<thead> Basic Information</thead>
				<tbody>
					<tr>
						<td><span>Firstname :</span></td>
						<td style='text-align:left;'><input type="text" name="fname" value="<?php echo $user["fname"];?>" ></td>
					</tr>

					<tr>
						<td><span>Lastname :</span></td>
						<td style='text-align:left;'><input type="text" name="lname" value="<?php echo $user["lname"];?>"></td>
					</tr>

					<tr>
						<td><span>E-Mail Address :</span></td>
						<td style='text-align:left;'><input type="text" name="email" value="<?php echo $user["email"]; ?>" readonly=""></td>
					</tr>
					<tr>
						<td><span>Phone Number :</span></td>
						<td style='text-align:left;'><input type="text" name="phonenumber" maxlength="11" value="<?php echo $user["phonenumber"];?>" ></td>
					</tr>
					<tr>
						<td><span>Address :</span></td>
						<td style='text-align:left;'><input type="text" name="address" value="<?php echo $user["address"];?>"></td>
					</tr>

					<tr>
						<td><span>Birthday :</span></td>
						<td style='text-align:left;'><input type="date" name="birthday" min="1970-01-01" max="2003-01-01" value="<?php echo $user["birthday"];?>"></td>
					</tr>
					...
					<tr>
						<td colspan="2">
							<input type="submit" name="update" value="Update Info" style='height: auto; width: 150; margin-right: auto; margin-left: auto; font-size: 16px; text-align: center; background: #131313; color: #E1E1E1; border-radius: 5px; cursor: pointer; padding: 10px; '>
			...
			<a href="user_profile.php" style='height: 35px; width: 150px; margin-right: auto; margin-left: auto; text-align: center; background: #131313; color: #E1E1E1; border-radius: 3px; cursor: pointer; text-decoration: none; line-height: 2.3; padding: 10px;'> Go Back To Your Profile</a> <br><br> 
